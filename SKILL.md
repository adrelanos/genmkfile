...
- It writes debhelper residue into the source tree (`debian/.debhelper`,
  ...
  "Empty directory found!". `genmkfile distclean` and `genmkfile deb-cleanup` both
  clear it.
...
- `genmkfile dist` errors "Empty directory found!" for TWO different reasons -- check which:
  ...
  - Build residue from a previous in-place `debuild`: `debian/.debhelper/generated/_source/home` is left behind empty. Clear it with `genmkfile deb-cleanup` (or `distclean`, which also drops debhelper residue). Note `deb-pkg` already runs `deb-cleanup` first, so this usually self-corrects on the next build.
...
- `deb-clean` (delete temporary debhelper files) / `deb-cleanup` (same, plus debuild artifacts in the parent folder). `distclean` runs the Makefile `clean` plus `deb-clean`.
- `lintian`.
